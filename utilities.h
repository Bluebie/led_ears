...
#define DebounceDuration 10 /* milliseconds */
#define Debounce(time, state) \
  static unsigned int start_time; \
  static byte check_state; \
  if (check_state != state) { start_time = time; } \
  check_state = state; \
  if ((unsigned int)(time - start_time) < DebounceDuration) return;
...
RGBPixel multiply_colors(RGBPixel left_op, RGBPixel right_op) {
  RGBPixelUnion left;
  RGBPixelUnion right;
  left.raw = left_op;
  right.raw = right_op;
...
RGBPixel maximum_mix_colors(RGBPixel left_op, RGBPixel right_op) {
  RGBPixelUnion left;
  RGBPixelUnion right;
  left.raw = left_op;
  right.raw = right_op;
  
  if (right.pixel.red > left.pixel.red)     left.pixel.red   = right.pixel.red;
  if (right.pixel.green > left.pixel.green) left.pixel.green = right.pixel.green;
  if (right.pixel.blue > left.pixel.blue)   left.pixel.blue  = right.pixel.blue;
  return left.raw;
}
...
unsigned int sp_random(unsigned int seed) {
